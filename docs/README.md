...
private IELTS study web app whose one live module is the Dictation Lab (practice
English listening by transcribing YouTube video segments, with token-level
correction, spaced-repetition review, analytics, and AI debriefs).
...
| 03 | [03-data-model.md](./03-data-model.md) | The 11 Mongoose collections, an ER diagram, per-model field tables and indexes, every status/enum union, and the guest-vs-user ownership split. |
...
- Layering (dependencies point downward): App Router pages and
  ...
  `src/modules/dictation/services` (decisions + record I/O) -> `src/models`
