...
ships one live learning module, the Dictation Lab, where the learner practices
English listening by transcribing short segments of YouTube videos, gets their
typed answer graded token by token, builds a spaced-repetition review queue from
their mistakes, and can receive an AI-generated debrief after a session. The app
is designed to run and degrade gracefully even when optional services (MongoDB,
Google auth, YouTube API, OpenAI, Cloudinary) are not configured.
...
| Vocabulary | `vocabulary` | future | `/vocabulary` |
...
Only the Dictation Lab is implemented; the others are placeholders that shape
